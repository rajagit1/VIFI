...
export class ActorDetailsPage implements OnInit {
  ...
  ionViewDidEnter() {
   ...
   let newVisitor :any;
    ...
    this.safeURLs = [];
    if(Object.keys(this.actorData).length !== 0){
    ...
    this.firebase.filterPostByActorName(requestedUserName).then(element => {
      ...
      if (!_.isEmpty(element['docs'])) {
        element['docs'].forEach(arg => {
                ...
                this.finalPostedStories.push(arg.data());
              });
           }
  ...
  async goToDetail(arg) {
    ...
    await loading.present();
    if (arg !== null) {
      if(arg.storyVisibility == 'Certified'){
        this.presentToast("Visible to Certified Producer","toast-danger");
        return;
      }
