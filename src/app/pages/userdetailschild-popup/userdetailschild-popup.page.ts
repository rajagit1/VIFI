...
@Component({
  selector: 'app-userdetailschild-popup',
  templateUrl: './userdetailschild-popup.page.html',
  styleUrls: ['./userdetailschild-popup.page.scss'],
})
export class UserdetailschildPopupPage implements OnInit {
  ...
  ionViewDidEnter() {
   ...
   let newVisitor :any;
    ...
    this.filterStories();
    this.filterPosts(this.actorData.actorName);
    this.showSkeltonLoading();
    setTimeout(() => {
      this.fetchComments(this.actorData['actorName']);
    }, 1000);
  }
  async filterPosts(requestedUserName){ 
    this.firebase.filterPostByActorName(requestedUserName).then(element => {
     
      if (!_.isEmpty(element['docs'])) {
        element['docs'].forEach(arg => {
                this.postedStories =[]              
                this.postedStories["title"] =arg.data().title;              
                this.postedStories["image"] =arg.data().image;
                this.postedStories["synopsis"] =arg.data().synopsis;
                this.postedStories["likes"] =arg.data().likes;
                this.postedStories["views"] = arg.data().views;
                this.finalPostedStories.push(this.postedStories);
              });
           }
